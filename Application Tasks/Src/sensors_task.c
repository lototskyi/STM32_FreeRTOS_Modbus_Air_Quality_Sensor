 ...
 * from various sensors at a specific interval and communicates with the Modbus Data Manager.
 */
 ...
 * It also handles messages for updating the sampling interval dynamically and can be extended to handle other type of messages.
 */
        ...
        if (sensors_task_has_message() == pdTRUE)
        {
            if (xQueueReceive(sensors_task_queue_handle, &received_msg, portMAX_DELAY) == pdTRUE)
            {
                switch (received_msg.msg_id)
                {
                    ...
                    default:
                        // Handle unknown message id if necessary
                        break;
                }
            }
