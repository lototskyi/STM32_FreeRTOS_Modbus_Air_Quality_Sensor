            ...
            switch (current_coil_address)
            {
                case COIL_NO_OPERATION:
                    ...
                    else
                    {
                        ...
                        // Example: Turn off a specific feature or stop a process.
                    }
    ...
    for (uint16_t i = 0; i < num_regs && status == ERR_OK; i++)
    {
        ...
        if (modbus_data_get_holding_register(current_reg_address, &register_value) == ERR_OK)
        {
            ...
            switch (current_reg_address)
            {
                ...
                // Expand for other holding registers as neeedd...
...
static error_t process_input_registers_update(modbus_data_mgr_processing_msg_t data_update_msg)
{
    ...
    {
        uint16_t current_reg_address = start_address + i;
        uint16_t register_value = *((uint16_t*)data_update_msg.data + i);
